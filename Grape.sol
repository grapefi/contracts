...
contract Grape is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("grape.finance", "GRAPE") {
    ...
    function _updateTaxRate(uint256 _grapePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_grapePrice >= taxTiersTwaps[tierId]) {
    ...
    function setGrapeOracle(address _grapeOracle) public onlyOperatorOrTaxOffice {
        require(_grapeOracle != address(0), "oracle address cannot be 0 address");
        _grapeOracle = _grapeOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGrapePrice = _getGrapePrice();
            currentTaxRate = _updateTaxRate(currentGrapePrice);
            if (currentGrapePrice < burnThreshold) {
                burnTax = true;
            }
        }
